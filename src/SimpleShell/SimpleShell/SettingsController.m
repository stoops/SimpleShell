...
- (void)windowDidLoad {
    ...
    NSView *mainView = wind.contentView;

    [mainView setWantsLayer:YES];

    NSStackView *subsView = [[NSStackView alloc] init];
    subsView.orientation = NSUserInterfaceLayoutOrientationVertical;
    subsView.distribution = NSStackViewDistributionFill;
    subsView.alignment = NSLayoutAttributeCenterX;
    subsView.spacing = 12.0;
    subsView.edgeInsets = NSEdgeInsetsMake(20, 20, 20, 20);
    subsView.translatesAutoresizingMaskIntoConstraints = NO;

    [mainView addSubview:subsView];

    [NSLayoutConstraint activateConstraints:@[
        [subsView.topAnchor constraintEqualToAnchor:mainView.topAnchor],
        [subsView.bottomAnchor constraintEqualToAnchor:mainView.bottomAnchor],
        [subsView.leadingAnchor constraintEqualToAnchor:mainView.leadingAnchor],
        [subsView.trailingAnchor constraintEqualToAnchor:mainView.trailingAnchor]
    ]];
    ...
    [subsView addView:rotView inGravity:NSStackViewGravityCenter];
    ...
    [subsView addView:sysView inGravity:NSStackViewGravityCenter];

    self.bgdColor = [self addRowToStack:subsView label:@"Background Color:" action:@selector(bgdChanged:)];
    self.tabColor = [self addRowToStack:subsView label:@"       Tab Color:" action:@selector(tabChanged:)];
    self.txtColor = [self addRowToStack:subsView label:@"      Text Color:" action:@selector(txtChanged:)];
    self.selColor = [self addRowToStack:subsView label:@" Highlight Color:" action:@selector(selChanged:)];
    self.crsColor = [self addRowToStack:subsView label:@"    Cursor Color:" action:@selector(crsChanged:)];
    self.inpColor = [self addRowToStack:subsView label:@"    Bottom Color:" action:@selector(inpChanged:)];
    ...
    [subsView addView:sliderView inGravity:NSStackViewGravityCenter];
    ...
    self.nopString = [[NSTextField alloc] init];
    self.nopString.drawsBackground = NO;
    self.nopString.focusRingType = NSFocusRingTypeNone;
    self.nopString.textColor = [NSColor clearColor];
    [self.nopString.widthAnchor constraintEqualToConstant:150].active = YES;

    [hideView addView:hideLabel inGravity:NSStackViewGravityLeading];
    [hideView addView:self.nopString inGravity:NSStackViewGravityTrailing];
    [subsView addView:hideView inGravity:NSStackViewGravityCenter];
    ...
    [subsView addView:seprView inGravity:NSStackViewGravityCenter];

    [mainView layoutSubtreeIfNeeded];
    [wind recalculateKeyViewLoop];
}
...
- (void)blrSliderChanged:(NSSlider *)sender {
    ...
    self.nopString.stringValue = [NSString stringWithFormat:@"%f", factor];
