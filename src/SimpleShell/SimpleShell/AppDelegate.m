        ...
        if (init != 1) {
            ...
            if ([wino.contentViewController isKindOfClass:[ViewController class]]) {
                ...
                if (vcon != nil) {
                    ...
                    NSColor *defTab = [NSColor colorWithRed:0.21 green:0.45 blue:0.69 alpha:0.75];
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown handler:^NSEvent * _Nullable(NSEvent * _Nonnull event) {
        ...
        if ([responder isKindOfClass:NSClassFromString(@"CursorText")] || (responder == wino.contentViewController.view)) {
            [self inpt:event];
            return nil;
        }
        return event;
