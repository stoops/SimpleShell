    ...
    if (prop == VTERM_PROP_ALTSCREEN) {
        ...
        @synchronized (self) {
            ...
            if (gridMode) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (self.vcon != nil) {
                        [self.vcon show:@"prop" data:nil pref:nil indx:self.vidx.intValue sels:2 cidx:-1 rows:self.rows.intValue cols:self.cols.intValue];
...
- (void)halt:(NSString *)pref {
    ...
    NSLog(@"HALT [%@] [%d] [%d] [%d]", pref, self.vidx.intValue, tabn, self.stop.intValue);
