...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
    ...
    if ([[pboard types] containsObject:SSAppTabPasteboardType]) {
        NSString *tagStr = [pboard stringForType:SSAppTabPasteboardType];
        NSInteger srcTag = [tagStr integerValue];
        NSInteger dstTag = self.tag;

        if ((srcTag != dstTag) && (self.target != nil)) {
            SEL moveSelector = NSSelectorFromString(@"moveTabSrcTag:dstTag:");
            if ([self.target respondsToSelector:moveSelector]) {
                #pragma clang diagnostic push
                #pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                [self.target performSelector:moveSelector withObject:@(srcTag) withObject:@(dstTag)];
                #pragma clang diagnostic pop
                return YES;
            }
        }
